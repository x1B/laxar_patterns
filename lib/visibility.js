      ...
      if( options.onAnyAreaRequest ) {
         ...
         scope.eventBus.subscribe( requestEvent, function( event ) {
            ...
            if( result === true || result === false ) {
               var didEvent = [ 'didChangeAreaVisibility', event.area ].join( '.' );
               scope.eventBus.publish( didEvent, { area: event.area, visible: result } );
            }
         } );
      }

      var self = this;
      if( options.onChange || options.onShow || options.onHide ) {
         var didEvent = [ 'didChangeAreaVisibility', scope.widget.area ].join( '.' );
         self.scope_.eventBus.subscribe( didEvent, function( event ) {
            var wasVisible = self.scope_.isVisible || false;
            self.scope_.isVisible = event.visible;
