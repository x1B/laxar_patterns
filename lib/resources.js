...
define( [
   ...
   function updateHandler( scope, modelKey ) {
      ...
      return function didUpdateHandler( event ) {
         if( isEmpty( event.data ) && isEmpty( event.updates ) && resourceBucket[ modelKey ] == null ) {
            return false;
         }
         ...
         if( 'data' in event ) {
            if( resourceBucket[ modelKey ] && event.data ) {
               resourceBucket[ modelKey ] = ax.object.options( resourceBucket[ modelKey ], event.data );
            }
            else {
               resourceBucket[ modelKey ] = event.data;
            }
